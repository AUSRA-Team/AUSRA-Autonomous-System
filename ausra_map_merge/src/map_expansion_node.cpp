...
//   robot_offset_x/y is the robot's GLOBAL spawn position in Gazebo.
//
...
//
// INIT_POSE CONFIGURATION (map_merge_params.yaml):
...
//
// PARAMETERS:
//   input_topic       — Raw SLAM map topic  (e.g. /ausra_1/map)
...
//   robot_offset_x    — Robot Gazebo spawn X in m (default:   0.0)
//   robot_offset_y    — Robot Gazebo spawn Y in m (default:   0.0)
...
public:
    ...
    frame_id_("map")         // safe default; overwritten on first SLAM message
  {
    ...
    this->declare_parameter<std::string>("input_topic",      "/ausra_1/map");
...
private:
  ...
  void mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr incoming_map)
  {
    ...
    //
    // slam_toolbox publishes info.origin in the robot's LOCAL SLAM frame
    // (ausra_X_map). robot_offset_x/y is the robot's Gazebo spawn position
    // in the GLOBAL frame.
    //
  ...
  double      robot_offset_x_;    ///< Robot's Gazebo spawn X in global frame
  double      robot_offset_y_;    ///< Robot's Gazebo spawn Y in global frame
